 ...
 * file into one of the F256 K2's extended 512 KiB RAM blocks.  Then pass
 ...
 *
 * Extended RAM blocks on F256 K2:
 ...
 *
 * Reads the file in chunks via kernelReadC and writes bytes into high memory via movedown24.
 *
 * Returns true on success (at least one byte read), false if the file could
 * not be opened.
 *
 * __attribute__((noinline)) needed to prevent optimizer related register allocation bug
 */
__attribute__((noinline))
bool vgm_himem_load(const char *path, uint32_t base_addr, vgm_himem_ctx_t *ctx);

/* vgm_read_fn-compatible callback: load high memory into cpu memory buf. */
...
/* Low-level 65816 MVN block copy; */
