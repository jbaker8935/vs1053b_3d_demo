...
void setup_model_vertices(const Model3D *model, uint8_t slot) {
    uint16_t base = VS_GEOM_SAVE_AREA_X + (uint16_t)slot * VS_GEOM_SAVE_SLOT_SIZE;
    vs1053_write_mem(base + VS_GEOM_SLOT_N_VERTICES, model->vertex_count);
    vs1053_write_sci(SCI_WRAMADDR, base + VS_GEOM_SLOT_INPUT_VERT);
...
bool no_near_far_coloring = false;
bool g_hidden_line_active = false;

void hidden_line_disable(void) {
    g_hidden_line_active = false;
    vs1053_write_mem(VS_GEOM_ENABLE_HIDDEN_LINE, 0x0000);
}

void hidden_line_enable(void) {
    g_hidden_line_active = true;
    vs1053_write_mem(VS_GEOM_ENABLE_HIDDEN_LINE, 0x0001);
}
...
void setup_model_hidden_line(const Model3D *model, uint8_t slot) {
    ...
    if (face_count > VS_GEOM_MAX_FACES) {
        face_count = VS_GEOM_MAX_FACES;
    }
    ...
    if (!has_hidden_data) {
        // clear face count.
        vs1053_write_mem(base + VS_GEOM_SLOT_N_FACES, 0x0000);
        return;
    }

    uint8_t edge_count = model->edge_count;
    if (edge_count > VS_GEOM_MAX_INPUT_EDGES) {
        edge_count = VS_GEOM_MAX_INPUT_EDGES;
    }

    vs1053_write_sci(SCI_WRAMADDR, base + VS_GEOM_SLOT_EDGE_FACE_MAP);
    for (uint8_t i = 0; i < edge_count; ++i) {
        uint8_t face0 = model->edge_face0[i];
        uint8_t face1 = model->edge_face1[i];
        vs1053_write_sci(SCI_WRAM, ((uint16_t)face1 << 8) | (uint16_t)face0);
    }

    vs1053_write_sci(SCI_WRAMADDR, base + VS_GEOM_SLOT_FACE_NORMALS);
    for (uint8_t i = 0; i < face_count; ++i) {
        vs1053_write_sci(SCI_WRAM, (uint16_t)model->face_nx[i]);
        vs1053_write_sci(SCI_WRAM, (uint16_t)model->face_ny[i]);
        vs1053_write_sci(SCI_WRAM, (uint16_t)model->face_nz[i]);
    }

    // Store face count 
    vs1053_write_mem(base + VS_GEOM_SLOT_N_FACES, face_count);
}

void geometry_kernel_apply_hidden_line(const Model3D *model, uint8_t slot) {
    setup_model_hidden_line(model, slot);
}
...
void load_model_to_plugin(const Model3D *model, uint8_t slot) {
    ...
    setup_model_vertices(model, slot);
    setup_model_edges(model, slot);
    setup_model_hidden_line(model, slot);
}

void setup_model_edges(const Model3D *model, uint8_t slot) {
    uint16_t base = VS_GEOM_SAVE_AREA_X + (uint16_t)slot * VS_GEOM_SAVE_SLOT_SIZE;
    vs1053_write_mem(base + VS_GEOM_SLOT_N_EDGES, model->edge_count);
    vs1053_write_sci(SCI_WRAMADDR, base + VS_GEOM_SLOT_EDGE_LIST);
...
bool geometry_kernel_save(uint16_t slot) {
    ...
    if (geometry_kernel_wait_complete(1000) == 1) {  // wait for completion (or error)
        return true;
    }
    return false;
}
...
uint8_t scr_x_lo[VS_GEOM_MAX_VERTICES];  // screen X low  bytes — for original vertices
uint8_t scr_x_hi[VS_GEOM_MAX_VERTICES];  // screen X high bytes
uint8_t scr_y[VS_GEOM_MAX_VERTICES];     // screen Y        (only low byte used)
...
uint16_t scene_screen_x[VS_GEOM_SCENE_MAX_VERTS];
uint16_t scene_screen_y[VS_GEOM_SCENE_MAX_VERTS];
int16_t scene_clip_x[VS_GEOM_SCENE_MAX_CLIPS];
int16_t scene_clip_y[VS_GEOM_SCENE_MAX_CLIPS];
...
uint16_t g_edge_buf_packed[VS_GEOM_MAX_EDGES];
uint8_t  g_edge_buf_flags[VS_GEOM_MAX_EDGES];

// Forward declaration of the monolithic asm emitter.
extern uint8_t get_screen_edges_full_asm(uint8_t layer);
...
void capture_plugin_state(PluginCapture *cap) {
    // camera inputs
    cap->cam_pitch_yaw = vs1053_read_mem(VS_GEOM_CAM_PITCH_YAW);
    cap->cam_roll_scale = vs1053_read_mem(VS_GEOM_CAM_ROLL_SCALE);
    cap->cam_pos_x = (int16_t)vs1053_read_mem(VS_GEOM_CAM_POS_X);
    cap->cam_pos_y = (int16_t)vs1053_read_mem(VS_GEOM_CAM_POS_Y);
    cap->cam_pos_z = (int16_t)vs1053_read_mem(VS_GEOM_CAM_POS_Z);
    cap->cam_sx = (int16_t)vs1053_read_mem(VS_GEOM_CAM_SX);
    cap->cam_cx = (int16_t)vs1053_read_mem(VS_GEOM_CAM_CX);
    cap->cam_sy = (int16_t)vs1053_read_mem(VS_GEOM_CAM_SY);
    cap->cam_cy = (int16_t)vs1053_read_mem(VS_GEOM_CAM_CY);
    cap->cam_sz = (int16_t)vs1053_read_mem(VS_GEOM_CAM_SZ);
    cap->cam_cz = (int16_t)vs1053_read_mem(VS_GEOM_CAM_CZ);

    // matrix (Y-RAM contiguous)
    uint16_t addr = VS_GEOM_MATRIX_BASE;
    for (int i = 0; i < 12; ++i) {
        cap->matrix[i] = (int16_t)vs1053_read_mem(addr++);
    }

    // projection params
    cap->proj_focal = vs1053_read_mem(VS_GEOM_PROJ_FOCAL);
    cap->proj_half_w = vs1053_read_mem(VS_GEOM_PROJ_HALF_W);
    cap->proj_half_h = vs1053_read_mem(VS_GEOM_PROJ_HALF_H);
    cap->proj_near_z = (int16_t)vs1053_read_mem(VS_GEOM_PROJ_NEAR_Z);

    // counts / status
    cap->n_vertices = vs1053_read_mem(VS_GEOM_N_VERTICES);
    cap->n_edges = vs1053_read_mem(VS_GEOM_N_EDGES);
    cap->n_output_edges = vs1053_read_mem(VS_GEOM_N_OUTPUT_EDGES);
    cap->n_clip_verts = vs1053_read_mem(VS_GEOM_N_CLIP_VERTS);
    cap->status = vs1053_read_mem(VS_GEOM_STATUS);
    ...
    // packed v0v1 at OUTPUT_EDGE_PACKED.
    {
        ...
        for (int i = 0; i < (int)n; ++i) {
            cap->edge_list[i]  = vs1053_read_mem(VS_GEOM_OUTPUT_EDGE_PACKED + (uint16_t)i);
            uint16_t fw = vs1053_read_mem(VS_GEOM_OUTPUT_EDGE_FLAGS + (uint16_t)(i >> 1));
    ...
    {
        uint16_t base = VS_GEOM_CLIP_SCREEN;
        for (int i = 0; i < cap->n_clip_verts && i < CAPTURE_MAX_CLIP_V; ++i) {
            cap->clip_screen[i * 2 + 0] = vs1053_read_mem(base + (uint16_t)(i * 2));
            cap->clip_screen[i * 2 + 1] = vs1053_read_mem(base + (uint16_t)(i * 2 + 1));
        }
    }
}
...
uint8_t get_screen_edges_with_depth(Model3D *model, uint8_t layer) {
    ...
    return get_screen_edges_full_asm(layer);
}
...
uint8_t get_screen_edges(Model3D *model, uint8_t color) {
    ...
    for (uint8_t i = 0; i < n_input; ++i) {
        ...
        screen_y[i] = vs1053_read_sci(SCI_WRAM) & 0x00FF;
    }
...

    {
        ...
        // Phase 1: burst-read all flags from VS_GEOM_OUTPUT_EDGE_FLAGS,
        // then use model edge data for unclipped edges and individual SCI reads for clipped.
        {
            uint8_t n_flag_words = (edge_count + 1) >> 1;
            vs1053_write_sci(SCI_WRAMADDR, VS_GEOM_OUTPUT_EDGE_FLAGS);
            for (uint8_t i = 0; i < n_flag_words; ++i) {
                uint16_t fw = vs1053_read_sci(SCI_WRAM);
            ...
            for (uint8_t e = 0; e < edge_count; ++e) {
                ...
                // Always read from VS_GEOM_OUTPUT_EDGE_PACKED: output edge index e does
                // NOT correspond to input edge e when culling reduces output count.
                uint16_t packed = vs1053_read_mem(VS_GEOM_OUTPUT_EDGE_PACKED + e);
...
// =============================================================================
// These functions program the VS_GEOM_SCENE_* memory region so the DSP scene
...
void scene_enable(void) {
    vs1053_write_mem(VS_GEOM_SCENE_ENABLE, 0x0001);
}

void scene_disable(void) {
    vs1053_write_mem(VS_GEOM_SCENE_ENABLE, 0x0000);
}

void scene_no_occlusion_enable(void) {
    vs1053_write_mem(VS_GEOM_SCENE_FLAGS, VS_GEOM_SCENE_FLAG_NO_OCCLUSION);
}

void scene_no_occlusion_disable(void) {
    vs1053_write_mem(VS_GEOM_SCENE_FLAGS, 0x0000);
}

void scene_set_object_count(uint8_t n_objects) {
    if (n_objects > VS_GEOM_SCENE_MAX_OBJECTS) {
        n_objects = VS_GEOM_SCENE_MAX_OBJECTS;
    }
    vs1053_write_mem(VS_GEOM_SCENE_N_OBJECTS, (uint16_t)n_objects);
}

void scene_set_object(uint8_t index, const SceneObjectParams *obj) {
    if (index >= VS_GEOM_SCENE_MAX_OBJECTS) {
        return;
    }
    uint16_t addr = VS_GEOM_SCENE_OBJ_PARAMS +
                    (uint16_t)index * VS_GEOM_SCENE_OBJ_STRIDE;
    // Write the 6-word per-object record:
    //   [slot_idx, pitch_yaw, roll_scale, pos_x, pos_y, pos_z]
    vs1053_write_mem(addr, obj->slot);
    vs1053_write_sci(SCI_WRAM, (uint16_t)obj->pitch << 8 | obj->yaw);
    vs1053_write_sci(SCI_WRAM, (uint16_t)obj->roll << 8 | obj->scale);
    vs1053_write_sci(SCI_WRAM, (uint16_t)obj->pos_x);
    vs1053_write_sci(SCI_WRAM, (uint16_t)obj->pos_y);
    vs1053_write_sci(SCI_WRAM, (uint16_t)obj->pos_z);
}
...
void scene_set_descriptor(uint8_t n_objects, const SceneObjectParams *objects) {
    ...
    scene_set_object_count(n_objects);
    for (uint8_t i = 0; i < n_objects; ++i) {
        scene_set_object(i, &objects[i]);
    }
}
...
void scene_get_result(SceneResult *result) {
    result->total_verts = vs1053_read_mem(VS_GEOM_SCENE_TOTAL_VERTS);
    result->total_edges = vs1053_read_mem(VS_GEOM_SCENE_TOTAL_EDGES);
    result->total_clips = vs1053_read_mem(VS_GEOM_SCENE_TOTAL_CLIPS);
    result->n_objects   = (uint8_t)vs1053_read_mem(VS_GEOM_SCENE_N_OBJECTS);
}
...
void scene_get_object_meta(uint8_t index, SceneObjectMeta *meta) {
    ...
    meta->vert_offset = vs1053_read_mem(VS_GEOM_SCENE_VERT_OFFSET + index);
    meta->edge_offset = vs1053_read_mem(VS_GEOM_SCENE_EDGE_OFFSET + index);
    meta->clip_offset = vs1053_read_mem(VS_GEOM_SCENE_CLIP_OFFSET + index);
    meta->vert_count  = vs1053_read_mem(VS_GEOM_SCENE_VERT_COUNT  + index);
    meta->edge_count  = vs1053_read_mem(VS_GEOM_SCENE_EDGE_COUNT  + index);
    meta->centroid_z  = (int16_t)vs1053_read_mem(VS_GEOM_SCENE_DEPTH + index);

    uint16_t aabb_addr = VS_GEOM_SCENE_AABB +
                         (uint16_t)index * VS_GEOM_SCENE_AABB_STRIDE;
    meta->aabb_min_x  = (int16_t)vs1053_read_mem(aabb_addr);
    meta->aabb_max_x  = (int16_t)vs1053_read_mem(aabb_addr + 1);
    meta->aabb_min_y  = (int16_t)vs1053_read_mem(aabb_addr + 2);
    meta->aabb_max_y  = (int16_t)vs1053_read_mem(aabb_addr + 3);
}

uint16_t scene_get_object_edge_offset(uint8_t index) {
    if (index >= VS_GEOM_SCENE_MAX_OBJECTS) {
        return 0;
    }
    return vs1053_read_mem(VS_GEOM_SCENE_EDGE_OFFSET + index);
}

uint16_t scene_get_object_edge_count(uint8_t index) {
    if (index >= VS_GEOM_SCENE_MAX_OBJECTS) {
        return 0;
    }
    return vs1053_read_mem(VS_GEOM_SCENE_EDGE_COUNT + index);
}


uint16_t scene_read_screen_coords(uint16_t *sx_out, uint16_t *sy_out,
                                   uint16_t max_verts) {
    uint16_t total = vs1053_read_mem(VS_GEOM_SCENE_TOTAL_VERTS);
    if (total > max_verts) {
        total = max_verts;
    }
    if (total > VS_GEOM_SCENE_MAX_VERTS) {
        total = VS_GEOM_SCENE_MAX_VERTS;
    }
    vs1053_write_sci(SCI_WRAMADDR, VS_GEOM_SCENE_SCREEN_COORDS);
    for (uint16_t i = 0; i < total; ++i) {
        sx_out[i] = vs1053_read_sci(SCI_WRAM);
        sy_out[i] = vs1053_read_sci(SCI_WRAM);
    }
    return total;
}
...
uint16_t scene_read_output_edges(uint16_t *packed_out, uint16_t *flags_out,
                                  uint16_t max_edges) {
    uint16_t total = vs1053_read_mem(VS_GEOM_SCENE_TOTAL_EDGES);
    if (total > max_edges) {
        total = max_edges;
    }
    if (total > VS_GEOM_SCENE_MAX_EDGES) {
        total = VS_GEOM_SCENE_MAX_EDGES;
    }
    vs1053_write_sci(SCI_WRAMADDR, VS_GEOM_SCENE_OUTPUT_EDGES);
    for (uint16_t i = 0; i < total; ++i) {
        packed_out[i] = vs1053_read_sci(SCI_WRAM);
        flags_out[i]  = vs1053_read_sci(SCI_WRAM);
    }
    return total;
}
...
uint16_t scene_read_clip_screen(int16_t *sx_out, int16_t *sy_out,
                                 uint16_t max_clips) {
    uint16_t total = vs1053_read_mem(VS_GEOM_SCENE_TOTAL_CLIPS);
    if (total > max_clips) {
        total = max_clips;
    }
    if (total > VS_GEOM_SCENE_MAX_CLIPS) {
        total = VS_GEOM_SCENE_MAX_CLIPS;
    }
    vs1053_write_sci(SCI_WRAMADDR, VS_GEOM_SCENE_CLIP_SCREEN);
    for (uint16_t i = 0; i < total; ++i) {
        sx_out[i] = (int16_t)vs1053_read_sci(SCI_WRAM);
        sy_out[i] = (int16_t)(vs1053_read_sci(SCI_WRAM) & 0x00FF);
    }
    return total;
}

uint8_t scene_get_screen_edges(uint8_t n_objects,
    ...
    if (objects != NULL) {
        scene_set_descriptor(n_objects, objects);
    }
    trigger_geometry_kernel();

    if (geometry_kernel_wait_complete(2000) != 1) {
        scene_disable();
    ...
    SceneResult res;
    scene_get_result(&res);

    uint16_t n_verts = res.total_verts;
    uint16_t n_clips = res.total_clips;

    if (n_verts > VS_GEOM_SCENE_MAX_VERTS) n_verts = VS_GEOM_SCENE_MAX_VERTS;
    if (n_clips > VS_GEOM_SCENE_MAX_CLIPS) n_clips = VS_GEOM_SCENE_MAX_CLIPS;

    // Read combined scene screen and clip buffers once.
    // Per-edge near/far colour selection uses VS_GEOM_EDGE_NEAR which the DSP
    ...
    if (n_verts > 0) {
        ...
        for (uint16_t i = 0; i < n_verts; ++i) {
            scene_screen_x[i] = vs1053_read_sci(SCI_WRAM);
            scene_screen_y[i] = vs1053_read_sci(SCI_WRAM);
        }
    }
    ...
    if (n_clips > 0) {
        ...
        for (uint16_t i = 0; i < n_clips; ++i) {
            scene_clip_x[i] = (int16_t)vs1053_read_sci(SCI_WRAM);
            scene_clip_y[i] = (int16_t)(vs1053_read_sci(SCI_WRAM) & 0x00FF);
        }
    }
    ...
    for (uint8_t obj = 0; obj < res.n_objects; ++obj) {
        ...
        uint16_t e_off = scene_get_object_edge_offset(obj);
        uint16_t e_cnt = scene_get_object_edge_count(obj);
        if (e_cnt > VS_GEOM_MAX_EDGES) e_cnt = VS_GEOM_MAX_EDGES;

        // Read this object's output edges into the local buffer (one SCI burst).
        vs1053_write_sci(SCI_WRAMADDR,
                         VS_GEOM_SCENE_OUTPUT_EDGES + e_off * 2);
        for (uint16_t e = 0; e < e_cnt; ++e) {
            g_edge_buf_packed[e] = vs1053_read_sci(SCI_WRAM);
            g_edge_buf_flags[e]  = (uint8_t)(vs1053_read_sci(SCI_WRAM) & 0x00FF);
        }
        ...
        for (uint16_t e = 0; e < e_cnt; ++e) {
            ...
            if (flags & VS_GEOM_EDGE_NEAR) continue;
