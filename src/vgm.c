...
vgm_status_t vgm_service(vgm_player_t *p)
{
    ...
    for (;;) {
        ...
        /* Variable wait: read a 16-bit sample count and convert with
         * samples_to_ticks() to avoid a general 32-bit software multiply. */
        case 0x61u: {
            uint16_t samples = buf_get_le16(p);
            p->samples_elapsed += (uint32_t)samples;
            schedule_wait(p, samples_to_ticks(samples));
            return VGM_WAITING;
        }
